...
-- Partidas: usuarios solo ven partidas donde participan
DROP POLICY IF EXISTS "Users read own partidas" ON public.partidas;
CREATE POLICY "Users read own partidas" ON public.partidas FOR SELECT
  USING (host_id = auth.uid() OR guest_id = auth.uid());
DROP POLICY IF EXISTS "Users insert partida as host" ON public.partidas;
CREATE POLICY "Users insert partida as host" ON public.partidas FOR INSERT
  WITH CHECK (host_id = auth.uid());
DROP POLICY IF EXISTS "Users update partida if participant" ON public.partidas;
CREATE POLICY "Users update partida if participant" ON public.partidas FOR UPDATE
  USING (host_id = auth.uid() OR guest_id = auth.uid());

-- Transacciones: usuario ve solo sus transacciones
DROP POLICY IF EXISTS "Users read own transacciones" ON public.transacciones;
CREATE POLICY "Users read own transacciones" ON public.transacciones FOR SELECT
  USING (user_id = auth.uid());

-- 6. Función para crear perfil al registrarse
...
-- 7. RPC: procesar victoria (CRÍTICO - evita manipulación desde cliente)
...
BEGIN
  -- Obtener partida (solo si está jugando)
  ...
  -- Actualizar balance del ganador (no restamos del perdedor, la apuesta ya estaba comprometida)
  ...
  SELECT id, 'apuesta', -v_apuesta, p_partida_id, '{"tipo":"perdida"}'::jsonb
  FROM profiles WHERE id = v_perdedor_id;
...
-- 8. RPC: crear partida (genera numero_prohibido y room_code)
CREATE OR REPLACE FUNCTION public.crear_partida(
  p_apuesta DECIMAL DEFAULT 1
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Generar room_code único de 5 caracteres (alfanuméricos)
  LOOP
  ...
  -- Bloquear apuesta en balance (restar)
...
CREATE OR REPLACE FUNCTION public.unirse_partida(
  p_room_code TEXT
)
RETURNS JSONB
LANGUAGE plpgsql
...
-- 10. RPC: elegir número (el servidor verifica si es la bomba)
CREATE OR REPLACE FUNCTION public.elegir_numero(
  p_partida_id UUID,
  p_numero INTEGER
)
RETURNS JSONB
...
BEGIN
  ...
  IF p_numero < 20 OR p_numero > 50 THEN
  ...
  IF p_numero = v_partida.numero_prohibido THEN
    -- Tocó la bomba: el que eligió pierde, el rival gana
    v_perdedor := auth.uid();
...
-- 12. Realtime: en Supabase Dashboard → Database → Replication → activa "partidas"
