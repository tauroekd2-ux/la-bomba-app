-- Ejecuta esto en Supabase: SQL Editor → New query → Pegar → Run
-- Añade la columna tx_hash y la función confirmar_deposito_usuario con 3 parámetros

ALTER TABLE public.confirmaciones_deposito
  ADD COLUMN IF NOT EXISTS tx_hash TEXT;

COMMENT ON COLUMN public.confirmaciones_deposito.tx_hash IS 'Hash de la transacción en la red (obligatorio al confirmar).';
